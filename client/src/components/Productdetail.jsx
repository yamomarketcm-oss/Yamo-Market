...
const ProductDetail = () => {
  ...
  useEffect(() => {
    const fetch_ = async () => {
      setLoading(true);
      try {
        const res = await fetch(`https://yamo-market-server.vercel.app/api/market/getproductslug/${slug}`);
  ...
  useEffect(() => {
    const fetch_ = async () => {
      setLoading(true);
      try {
        const res = await fetch(`https://yamo-market-server.vercel.app/api/market/getshop/${product?.shop}`);
        if (!res.ok) throw new Error('Failed to fetch product');
        const result = await res.json();
        setShop(result);
      } catch (err) {
